...
export var ArticleList = memo(function (props) {
    ...
    if (isLoading) {
        return (_jsx("div", __assign({ className: classNames(cls.ArticleList, {}, [className, cls[view]]) }, { children: getSkeletons(view) })));
    }
    var renderArticle = function (article) { return (_jsx(ArticleListItem, { article: article, view: view, className: cls.card }, article.id)); };
    return (_jsx("div", __assign({ className: classNames(cls.ArticleList, {}, [className, cls[view]]) }, { children: articles.length > 0 ? articles.map(renderArticle) : null })));
});
